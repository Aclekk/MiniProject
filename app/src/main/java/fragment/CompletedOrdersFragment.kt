...
class CompletedOrdersFragment : Fragment() {
    ...
    // ✅ FIX: Pakai API seperti HistoryOrdersFragment
        ...
        viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
            try {
                val resp = ApiClient.apiService.getBuyerOrders(
                    token = "Bearer $token",
                    status = "completed" // ✅ Filter hanya completed
                )
                    ...
                    if (resp.isSuccessful) {
                        ...
                        if (body?.success == true && body.data != null) {
                            ...
                            if (orders.isEmpty()) {
                                ...
                                Toast.makeText(requireContext(), "Belum ada pesanan selesai", Toast.LENGTH_SHORT).show()
                            }
                        } else {
